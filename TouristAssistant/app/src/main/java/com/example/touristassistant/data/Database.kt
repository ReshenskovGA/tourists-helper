 ...
 * Обеспечивают преобразование сложных типов данных в примитивы для хранения в БД
 * и обратное преобразование при чтении с использованием Gson для JSON сериализации.
 */

class Converters {
    private val gson = Gson() // Инициализация Gson для JSON операций

    /**
     * Преобразует JSON-строку в список строк.
     * Используется для хранения списков (например, фотографий) в виде JSON в БД.
     *
     * @param value JSON-строка, содержащая список строк
     * @return десериализованный список строк или пустой список при null
     */
     ...
     * Сериализует список для хранения в текстовом поле БД.
     ...
     * Важно: RoutePoint должен быть сериализуемым Gson (data class).
